...
#	print('\n')
	...
	for x in range(0,count):
		...
		if today2 == created[0:10]:
			...
			#===============================
#			print("Is Subscribed Return Data")
#			memberEmail ='https://us12.api.mailchimp.com/3.0/search-members?query=@cooper'
			#memberEmail = config.api_root + 'search-members?query='+email


#			isSub = requests.get(memberEmail, auth=('apikey',config.apikey), 0)
#			print(isSub['exact_matches'])


			#======= Try having the request in the same form as the post request below in order to get info out of it
#			response = requests.get(memberEmail, auth=('apikey',config.apikey), data=payload)
#		      
			#print the response
#			try:
...
#			print("end of return")
			#isClient = client.search_members.get(memberEmail, auth=('apikey',config.apikey))
			#print(isClient)
#			print('endOfSearch')
			#===============================
...
#Uncomment to write	#print the response
#			try:
#				response.raise_for_status()
#				body = response.json()
#				id = body['id']
